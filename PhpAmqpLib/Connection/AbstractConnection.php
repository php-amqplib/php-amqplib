...
abstract class AbstractConnection extends AbstractChannel
{
    ...
    protected function connection_tune($args)
    {
        ...
        if ($this->config !== null) {
            $heartbeatTune = $this->config->getHeartbeatTune();
        } else {
            $heartbeatTune = AMQPConnectionConfig::HEARTBEAT_TUNE_DEFAULT;
        }

        switch ($heartbeatTune) {
            case AMQPConnectionConfig::HEARTBEAT_TUNE_LEGACY:
                if ($this->heartbeat > 0 && $v > 0) {
                    $this->heartbeat = min($this->heartbeat, $v);
                }
                break;
            case AMQPConnectionConfig::HEARTBEAT_TUNE_AUTO:
                if ($this->heartbeat === 0 || $v === 0) {
                ...
                break;
            case AMQPConnectionConfig::HEARTBEAT_TUNE_PREFER_BROKER:
                $this->heartbeat = $v;
                break;
            case AMQPConnectionConfig::HEARTBEAT_TUNE_PREFER_CLIENT:
                break;
        }
